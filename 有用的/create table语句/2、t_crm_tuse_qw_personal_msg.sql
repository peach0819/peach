CREATE TABLE `t_crm_tuse_qw_personal_msg` (
  ...
  `tuse_file_id` varchar(100) DEFAULT NULL COMMENT '涂色文件id',
  `tuse_msg_type` int(10) DEFAULT NULL COMMENT '涂色微信消息类型',
  ...
  `from_chat_id` bigint(10) DEFAULT NULL COMMENT '消息发送者crm系统chat_id',
  `to_chat_id` bigint(10) DEFAULT NULL COMMENT '消息接收者crm系统chat_id',
  PRIMARY KEY (`id`),
  KEY `idx_callback_id` (`callback_id`),
  KEY `idx_tuse_msg_id` (`tuse_msg_id`),
  KEY `idx_tuse_msg_time` (`tuse_msg_time`),
  KEY `idx_from_chat_id_to_chat_id` (`from_chat_id`,`to_chat_id`)
) ENGINE=InnoDB AUTO_INCREMENT=3022805 DEFAULT CHARSET=utf8mb4 COMMENT='涂色私聊消息'