...
insert overwrite table dw_salary_sign_brand_rule_public_d partition (dayid=''$v_date'',planno=''12587'',pltype=''pre'')
select
   from_unixtime(unix_timestamp(),''yyyy-MM-dd HH:mm:ss'') as update_time,--更新时间
   from_unixtime(unix_timestamp(),''yyyy-MM'') as update_month,--执行月份
   ''历史方案'' as plan_type ,--明细类型
   --方案基础信息,
   ''202107'' as plan_month,--方案月份
   ''2021-07-01~2021-07-31'' as plan_pay_time,--方案时间
   ''7月恩霈新签政策-江西省区-to主管'' as plan_name,--方案名称
...
select
    ...
    ''【辖区】BD主管'' as grant_object_type,
    --发放对象用户ID
            bd_manager_id as grant_object_user_id,
        --发放对象用户姓名
            bd_manager_name as grant_object_user_name,
        -- 发放对象部门ID
            bd_manager_dep_id as grant_object_user_dep_id,
...
--
   select
           ...
           sum(refund_retreat_amount) as refund_retreat_amount,--实货清退金额
                                    ...
                                    1000 as new_sign_line,
                case when sum(gmv_less_refund) >= 1000 then ''是'' else ''否'' end as is_over_sign_line--是否满足新签门槛
	   ...
	   select *
         ...
         where dayid=''20210731''
          and shop_brand_sign_day between ''20210701'' and ''20210731''
          and pay_day <=''20210731''
...
left join
( select user_id, substr(leave_time,1,8) as leave_time from dwd_user_admin_d where dayid=''20210731''
...
insert overwrite table dw_salary_backward_plan_sum_mid_d partition (dayid=''$v_date'',planno=''12587'')
select
	...
	sts_target,--统计指标数值
    --方案扩展字段
    concat(''grant_object_rk:'',nvl(case when ''累计阶梯返实物'' =''排名返现'' then grant_object_rk else null end,''''),''\;'',''grant_object_underling_cnt:'',nvl(case when sts_target_name in (''人均新签品牌门店数'',''人均新签GMV'',''人均新签支付金额'')  then  grant_object_underling_cnt else null end,'''')) as real_coefficient_goal_rate
    --提成上限
...
select
    ...
    select *
        ...
        where dayid =''$v_date'' and pltype=''cur'' and planno=''12587''
    ) t
    left join
...
;
    ...
    "
    if [ "202107" -ne "$v_cur_month" ]; then
        hive -e "

--历史方案汇总表
use ytdw;
insert overwrite table dw_salary_backward_plan_sum_d partition (dayid=''$v_date'',planno=''12587'')
select
...
then pre_plan.commission_reward else cur_plan.commission_reward end - pre_plan.commission_reward, 2)
 ...
 (
   select  * from dw_salary_backward_plan_sum_mid_d
    where dayid =''$v_pre_month_last_day'' and plan_no=''12587''
	) pre_plan

    left join
	( select  * from dw_salary_backward_plan_sum_mid_d
      where dayid =''$v_date''  and plan_no=''12587''
