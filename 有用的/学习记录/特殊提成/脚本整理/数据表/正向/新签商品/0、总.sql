...
insert overwrite table dw_salary_sign_item_rule_public_d partition (dayid='$v_date',planno='13492',pltype='cur')
select
...
insert overwrite table dw_salary_forward_plan_sum_d partition (dayid='$v_date',planno='13492')
select
    from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss') as update_time,--更新时间
    from_unixtime(unix_timestamp(),'yyyy-MM') as update_month,--执行月份
    --方案基础信息,
...
select
	...
	where dayid ='$v_date' and pltype='cur' and planno='13492'
	group by
	plan_month,--方案月份
    plan_pay_time,--方案时间
    planno,--方案编号
