...
insert overwrite table dw_salary_backward_plan_sum_mid_d partition (dayid='$v_date',planno='13165')
select
    from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss') as update_time,--更新时间
    from_unixtime(unix_timestamp(),'yyyy-MM') as update_month,--执行月份
    --方案基础信息
...
from (
        select
            ...
            where dayid ='$v_date' and pltype='pre' and planno='13165'
            group by
            plan_month,--方案月份
            plan_pay_time,--方案时间
            planno,--方案编号
...
then pre_plan.commission_reward else cur_plan.commission_reward end - pre_plan.commission_reward, 2)
         ...
         (
           select  * from dw_salary_backward_plan_sum_mid_d
            where dayid ='$v_pre_month_last_day' and plan_no='13165'
            ) pre_plan

            left join
            ( select  * from dw_salary_backward_plan_sum_mid_d
              where dayid ='$v_date'  and plan_no='13165'
