...
-- 当月明细 (每天的每条订单的数据)， 数据是订单维度
insert overwrite table dw_salary_gmv_rule_public_d partition (dayid='$v_date')
select
   from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss') as update_time,--更新时间
   from_unixtime(unix_timestamp(),'yyyy-MM') as update_month,--执行月份
   '当月方案' as plan_type ,--明细类型
   --方案基础信息,
        ,planno='12381',pltype='cur'
...
left join
(
    select user_id, substr(leave_time,1,8) as leave_time
...
--汇总表， 数据是人在这个月的合计，是以人和方案为维度
insert overwrite table dw_salary_forward_plan_sum_d partition (dayid='$v_date',planno='12381')
select
    from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss') as update_time,--更新时间
    from_unixtime(unix_timestamp(),'yyyy-MM') as update_month,--执行月份
    --方案基础信息
	...
	sts_target,--统计指标数值
    --方案扩展字段
    concat('grant_object_rk:',nvl(case when '累计阶梯返点' ='排名返现' then grant_object_rk else null end,''),'\;','grant_object_underling_cnt:','') as real_coefficient_goal_rate,
 ...
 (
    ...
    from (
        ...
        where dayid ='$v_date' and pltype='cur' and planno='12381'
        group by
            plan_month,--方案月份
            plan_pay_time,--方案时间
            planno,--方案编号
