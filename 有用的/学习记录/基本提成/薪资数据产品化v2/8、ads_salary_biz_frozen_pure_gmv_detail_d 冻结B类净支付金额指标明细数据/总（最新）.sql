...
CREATE TABLE IF NOT EXISTS ads_salary_biz_frozen_pure_gmv_detail_d (
  ...
  b_pure_pay_amount_freezed_not_pickup_to_shihuo decimal(11,2) COMMENT '冻结实货支付(非提货卡支付)',
...
(
    ...
    case  when (is_split='大BD' or is_split='BD') and is_pickup_recharge_order=0 and is_spec_brand=0 then (
          ...
          when (is_split='大BD' or is_split='BD') and is_pickup_recharge_order=1 and  is_spec_brand=0 then (
            case when (pickup_category_id_first=10 and is_celeron=1 and pickup_brand_id not in (17221,17697))
    	           ...
    	           when (pickup_category_id_first=542 and pickup_brand_id not in (13818,2068,7324,10726,9566,7888))
    		         ...
    		         when pickup_category_id_first=12 or pickup_category_id_third=11191 or pickup_brand_id=17221 then '分类03'
          ...
          when is_split='大BD' and is_pickup_recharge_order=0 and is_spec_brand=1 then (
            case when (category_id_first=10 and is_celeron=1 and brand_id not in (17221,17697))
        	       ...
        	       when (category_id_first=542 and brand_id not in (13818,2068,7324,10726,9566,7888))
                      or (category_id_first in (4,2794,2627,8,2681,11,2560,2750,5,11543,11544,11545,11546) and category_id_second not in (6820,2807)) then '分类06'
          ...
          when is_split='大BD' and is_pickup_recharge_order=1 and is_spec_brand=1 then (
            case when (pickup_category_id_first=10 and is_celeron=1 and pickup_brand_id not in (17221,17697))
          	     ...
          	     when (pickup_category_id_first=542 and pickup_brand_id not in (13818,2068,7324,10726,9566,7888))
                      or (pickup_category_id_first in (4,2794,2627,8,2681,11,2560,2750,5,11543,11544,11545,11546) and pickup_category_id_second not in (6820,2807)) then '分类06'
          		   when pickup_category_id_first=12 or pickup_category_id_third=11191 or pickup_brand_id=17221 then '分类07'
          	     else '分类08' end)
          else ''
    end as class_number,
    case
  ...
  (
    ...
    -- 员工离职信息
    left join
    (select user_id,leave_time from  dim_hpc_pub_user_admin) user
