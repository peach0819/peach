...
create table if not exists ads_salary_biz_kn_pure_gmv_d (
  ...
  big_a_coefficient_summary decimal(11,2) COMMENT 'A类大系数总和',
  small_a_coefficient_summary decimal(11,2) COMMENT 'A类小系数总和',
  big_a_coefficient_amount decimal(11,2) COMMENT 'A类大系数支付金额',
  small_a_coefficient_amount decimal(11,2) COMMENT 'A类小系数支付金额'
)
comment '库内净GMV'
...
select
    ...
    sum(case when business_group_name='跨境长尾' or business_unit in ('KA','品牌','美妆个护','长尾') then a_pure_pay_amount else 0 end)
    as small_a_coefficient_amount
from
    (
        select
            service_user_id,
