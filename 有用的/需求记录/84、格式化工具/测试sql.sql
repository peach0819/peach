...
FROM
  ...
  LEFT JOIN (
    SELECT
      shopOrder.shop_id,
      count(DISTINCT shopOrder.trade_id) AS refund_count,
      concat_ws(
        ',',
        collect_set(CAST(shopOrder.brand_id AS string))
      ) AS brand_ids
    FROM
      ytdw.dw_order_d shopOrder
      INNER JOIN ytdw.dwd_order_refund_all_d orderRefund ON shopOrder.refund_id = orderRefund.id
    WHERE
      shopOrder.dayid = '${v_date}'
  ...
  LEFT JOIN (
    SELECT
      shop_id,
      concat_ws(',', collect_set(CAST(brand_id AS string))) AS b_will_be_closed_brand,
      count(*) AS b_will_be_closed
    FROM
      yt_crm.st_shop_group_will_opt_d
    WHERE
      dayid = '${v_date}'
    GROUP BY
      shop_id
    ...
    SELECT
      ...
      SUM(
        ...
        CASE
          ...
          AND refund_status = 9 THEN refund_actual_amount
          ELSE 0
        END
      ) AS this_pure_gmv
    FROM
