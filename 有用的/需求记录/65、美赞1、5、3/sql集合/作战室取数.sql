...
SELECT data.user_id as `用户id`,
       ...
       nvl(data.month_visit_freq_valid_rate, '') as `当月门店拜访频次达成率`,
       nvl(data.month_visit_freq_valid_rate_numerator, '') as `当月门店拜访频次达成率分子`,
       nvl(data.month_visit_freq_valid_rate_denominator, '') as `当月门店拜访频次达成率分母`,
       nvl(data.month_nka_nc_visit_valid_rate, '') as `当月NKA专职NC门店拜访达成率`,
       nvl(data.month_nka_nc_visit_valid_rate_numerator, '') as `当月NKA专职NC门店拜访达成率分子`,
       nvl(data.month_nka_nc_visit_valid_rate_denominator, '') as `当月NKA专职NC门店拜访达成率分母`,
       nvl(data.month_rka_nc_visit_valid_rate, '') as `当月RKA专职NC门店拜访达成率`,
       nvl(data.month_rka_nc_visit_valid_rate_numerator, '') as `当月RKA专职NC门店拜访达成率分子`,
       nvl(data.month_rka_nc_visit_valid_rate_denominator, '') as `当月RKA专职NC门店拜访达成率分母`,
       nvl(data.month_shop_visit_valid_rate, '') as `当月门店拜访覆盖率`,
       nvl(data.month_shop_visit_valid_rate_numerator, '') as `当月门店拜访覆盖率分子`,
       nvl(data.month_shop_visit_valid_rate_denominator, '') as `当月门店拜访覆盖率分母`,
       nvl(data.month_fws_visit_valid_rate, '') as `当月服务商拜访达成率`,
       nvl(data.month_fws_visit_valid_rate_numerator, '') as `当月服务商拜访达成率分子`,
       nvl(data.month_fws_visit_valid_rate_denominator, '') as `当月服务商拜访达成率分母`,
       nvl(data.quar_fws_visit_valid_rate, '') as `当季服务商拜访达成率`,
       nvl(data.quar_fws_visit_valid_rate_numerator, '') as `当季服务商拜访达成率分子`,
       nvl(data.quar_fws_visit_valid_rate_denominator, '') as `当季服务商拜访达成率分母`,
       nvl(data.month_gt_shop_visit_valid_rate, '') as `当月GT渠道门店拜访覆盖率`,
       nvl(data.month_gt_shop_visit_valid_rate_numerator, '') as `当月GT渠道门店拜访覆盖率分子`,
       nvl(data.month_gt_shop_visit_valid_rate_denominator, '') as `当月GT渠道门店拜访覆盖率分母`,
       nvl(data.quar_gt_shop_visit_valid_rate, '') as `当季GT渠道门店拜访覆盖率`,
       nvl(data.quar_gt_shop_visit_valid_rate_numerator, '') as `当季GT渠道门店拜访覆盖率分子`,
       nvl(data.quar_gt_shop_visit_valid_rate_denominator, '') as `当季GT渠道门店拜访覆盖率分母`,
       nvl(data.month_gt_hospital_shop_visit_valid_rate, '') as `当月GT渠道院线店拜访覆盖率`,
       nvl(data.month_gt_hospital_shop_visit_valid_rate_numerator, '') as `当月GT渠道院线店拜访覆盖率分子`,
       nvl(data.month_gt_hospital_shop_visit_valid_rate_denominator, '') as `当月GT渠道院线店拜访覆盖率分母`,
       nvl(data.quar_gt_hospital_shop_visit_valid_rate, '') as `当季GT渠道院线店拜访覆盖率`,
       nvl(data.quar_gt_hospital_shop_visit_valid_rate_numerator, '') as `当季GT渠道院线店拜访覆盖率分子`,
       nvl(data.quar_gt_hospital_shop_visit_valid_rate_denominator, '') as `当季GT渠道院线店拜访覆盖率分母`,
       nvl(data.month_hospital_visit_valid_rate, '') as `当月院线店拜访覆盖率`,
       nvl(data.month_hospital_visit_valid_rate_numerator, '') as `当月院线店拜访覆盖率分子`,
       nvl(data.month_hospital_visit_valid_rate_denominator, '') as `当月院线店拜访覆盖率分母`
FROM data
