...
-- 历史明细
insert overwrite table dw_salary_gmv_rule_public_d partition (dayid='$v_date',planno='14473',pltype='pre')
select
...
left join
( select user_id, substr(leave_time,1,8) as leave_time from dwd_user_admin_d where dayid='20211231'
...
insert overwrite table dw_salary_backward_plan_sum_mid_d partition (dayid='$v_date',planno='14473')
select
    from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss') as update_time,--更新时间
    from_unixtime(unix_timestamp(),'yyyy-MM') as update_month,--执行月份
    --方案基础信息
...
from (
        select
                ...
                where dayid ='$v_date' and pltype='pre' and planno='14473'
            ) t
            left join (
...
if [ "202112" -ne "$v_cur_month" ]; then
    hive -e "
        use ytdw;
        insert overwrite table dw_salary_backward_plan_sum_d partition (dayid='$v_date',planno='14473')
        select
...
then pre_plan.commission_reward else cur_plan.commission_reward end - pre_plan.commission_reward, 2)
         ...
         (
           select  * from dw_salary_backward_plan_sum_mid_d
            where dayid ='$v_pre_month_last_day' and plan_no='14473'
            ) pre_plan

            left join
            ( select  * from dw_salary_backward_plan_sum_mid_d
              where dayid ='$v_date'  and plan_no='14473'
