...
case when coalesce(a.cur_commission_reward,'#') <> coalesce(b.cur_commission_reward,'#') then concat('存在差异【',a.cur_commission_reward,'：',b.cur_commission_reward,'】') else '无差异' end as cur_commission_reward_diff_flg,
case when coalesce(a.pre_commission_reward,'#') <> coalesce(b.pre_commission_reward,'#') then concat('存在差异【',a.pre_commission_reward,'：',b.pre_commission_reward,'】') else '无差异' end as pre_commission_reward_diff_flg,
case when coalesce(a.commission_reward_change,'#') <> coalesce(b.commission_reward_change,'#') then concat('存在差异【',a.commission_reward_change,'：',b.commission_reward_change,'】') else '无差异' end as commission_reward_change_diff_flg
 from ( select * from dw_salary_backward_plan_sum_d where dayid='$v_date' ) a

left join (select * from dw_salary_backward_plan_sum_new_d where dayid='$v_date' ) b on 1=1
