...
source ../yarn_variable.sh dw_salary_sign_item_rule_public_forward_d '肥桃'
...
select
    ...
    item.war_zone_id,
    item.war_zone_name,
    item.war_zone_dep_id,
    item.war_zone_dep_name,
    item.area_manager_id,
    item.area_manager_name,
    item.area_manager_dep_id,
    item.area_manager_dep_name,
    item.bd_manager_id,
    item.bd_manager_name,
    item.bd_manager_dep_id,
    item.bd_manager_dep_name,
    item.service_user_id_freezed,
    item.service_department_id_freezed,
    item.service_user_name_freezed,
    item.service_feature_name_freezed,
    item.service_job_name_freezed,
    item.service_department_name_freezed,
    item.new_sign_time,
    item.new_sign_day,
    item.new_sign_rn,
    item.shop_item_sign_day,
    item.gmv_less_refund,
    item.gmv,
    item.pay_amount,
    item.pay_amount_less_refund,
    item.refund_actual_amount,
    item.refund_retreat_amount,
    item.new_sign_line,
    item.is_over_sign_line,
    item.is_first_sign,
    item.is_succ_sign,
    item.grant_object_type,
    item.grant_object_user_id,
    item.grant_object_user_name,
    item.grant_object_user_dep_id,
    item.grant_object_user_dep_name,
    item.leave_time,
    item.is_leave,
    item.sts_target_name,
    item.planno
from (
    select
      *
  ...
  on item.planno = plan.no
  -- 关联不上的，就是不参与补数据
  where plan.no is null
  -- 非补数模式下自动跳过
  and 1='${supply_data_mode}'
)
insert overwrite table dw_salary_sign_item_rule_public_new_d partition (dayid='$v_date',pltype='cur')
select
           ...
           (select
              ...
              from
                  (select
                            ...
                            ---业务中间表
                       from (select * from dw_salary_sign_rule_public_mid_v2_d where dayid='$v_date') a
                      cross join bounty_plan2 b
                         ---过滤条件中的 全局过滤条件
                      where 1=1
                        and shop_item_sign_day between calculate_date_value_start and calculate_date_value_end
