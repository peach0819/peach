...
(select t1.no,
  ...
  where dayid ='$v_date'
    and is_deleted =0
    and t1.bounty_rule_type =3 --仅限新签品牌
    and status =1 --启用
...
(select t2.no ,
   ...
   from bounty_plan_payout t2
   left join (select * from dwd_bounty_indicator_d where dayid='$v_date') t3
   on t2.bounty_indicator_id=t3.id
)

insert overwrite table dw_salary_forward_plan_sum_new_d partition (dayid='$v_date',bounty_rule_type=3)
select
       ...
       ,payout_upper_limit as commission_cap --提成上限
       ...
       ,ytdw.bounty_payout(payout_rule_type,
                       ...
                       payout_upper_limit,
                       payout_config_json)  commission_reward, --提成奖品
        planno
       from
       (select
                --方案基础信息
                       ...
                       select
                        ...
                        inner join bounty_plan_payout2 t2
                         ...
                         left join
                           ...
                           on t1.grant_object_user_id=t3.user_id
                        group by
                              plan_month,--方案月份
                              plan_pay_time,--方案时间
                              planno,--方案编号
