...
source ../yarn_variable.sh dw_salary_sign_brand_rule_public_forward_d '肥桃'
...
where dayid ='$v_date'
  ...
  and status =1 --启用
  and month=concat(substr('$v_date',1,4),'-',substr('$v_date',5,2))--当月正向
),
 --bounty_plan字段补充
bounty_plan2 as
(select
...
select
    ...
    brand.planno
from (
    select * from dw_salary_sign_brand_rule_public_new_d
...
insert overwrite table dw_salary_sign_brand_rule_public_new_d partition (dayid='$v_date',pltype='cur')
select
 ...
 from
     (select
             ...
             grant_object_type as grant_object_type,
