...
if [ $supply_data_where_condition ]; then
...
source ../yarn_variable.sh dw_salary_backward_plan_sum_mid_sign_brand_rule_d '肥桃'
...
from (
    ...
    where dayid = '$v_date' and bounty_rule_type=3
  ) t1 left join (
  	select no
    from dw_bounty_plan_d t1
    where ${supply_data_where_condition}
...
insert overwrite table dw_salary_backward_plan_sum_mid_new_d partition (dayid='$v_date',bounty_rule_type=3)
select
       ...
       concat('grant_object_rk:',
               ...
               nvl(case when sts_target_name in ('人均新签品牌门店数','人均新签GMV','人均新签支付金额')  then  grant_object_underling_cnt else null end,
  ...
  from
       (select
          ...
          from (select
                      ...
                      ---统计指标----
                      --'统计指标名称为新签GMV'
                      case when indicator_code = 'NEW_SIGNING_BRAND_3_NEW_SING_SHOPS' then --'统计指标名称为新签门店数'
                                count(distinct case when is_leave='否' and is_succ_sign='是' then shop_id else null end )
                           when indicator_code = 'NEW_SIGNING_BRAND_3_NEW_SING_BRAND_SHOPS' then --统计指标名称为新签品牌门店数'
                                count(distinct case  when  is_leave='否' and is_succ_sign='是' then concat('_',shop_id,brand_id)  else null end )
                           when indicator_code = 'NEW_SIGNING_BRAND_3_ACC_NEW_SING_GMV' then--'统计指标名称为新签GMV'
                                sum(case when is_leave='否' and is_succ_sign='是' then gmv_less_refund else 0 end )
                           when indicator_code = 'NEW_SIGNING_BRAND_3_ACC_NEW_SING_PAY_AMT' then--'统计指标名称为累计新签支付金额'
                                sum(case when is_leave='否' and is_succ_sign='是' then pay_amount_less_refund else 0 end )
                           when indicator_code = 'NEW_SIGNING_BRAND_AVG_NEW_SING_SHOPS' then --人均新签品牌门店数'
                                count(distinct case  when  is_leave='否' and is_succ_sign='是' then concat('_',shop_id,brand_id)  else null end )/max(nvl(underling_cnt,1))
                           when indicator_code = 'NEW_SIGNING_BRAND_AVG_NEW_SING_GMV' then--'人均新签品GMV'
                                sum(case when is_leave='否' and is_succ_sign='是' then gmv_less_refund else 0 end )/max(nvl(underling_cnt,1))
                           when indicator_code = 'NEW_SIGNING_BRAND_AVG_NEW_SING_PAY_AMT' then --'人均新签支付金额'
                                sum(case when is_leave='否' and is_succ_sign='是' then pay_amount_less_refund else 0 end )/max(nvl(underling_cnt,1))
                 ...
                 left join
                   ...
                   on t.grant_object_user_id=t1.user_id
                  and t1.dayid = replace(last_day(concat(substr(t.plan_month,1,4),'-',concat(substr(t.plan_month,5,2)))),'-','') --方案月份的月末日期与下属人数表的dayid保持一致
