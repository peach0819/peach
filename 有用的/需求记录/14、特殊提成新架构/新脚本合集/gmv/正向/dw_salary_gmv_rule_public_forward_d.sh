...
select
     ...
     gmv.category_id_first_name,
      ...
      gmv.planno
from (
    select * from dw_salary_gmv_rule_public_new_d
...
insert overwrite table dw_salary_gmv_rule_public_new_d partition (dayid='$v_date',pltype='cur')
select
   ...
   select
    ...
    from
         (select *
           ...
           where dayid ='$v_date' and substr(pay_day,1,6) ='$v_cur_month' ) a
