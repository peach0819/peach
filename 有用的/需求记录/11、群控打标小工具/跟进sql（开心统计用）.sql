SELECT shop.shop_id as 门店id,
       ...
       total.qw_external_user_id as 客户企微外部id,
       customer.name as 客户微信昵称,

       total.qw_user_id as 添加人帐号,
       xiaoer.name as 添加人,
...
FROM (
    ...
    AND version = (SELECT max(version) FROM yt_crm.t_crm_chat_tag_bind WHERE version_type = 0)
) total

INNER JOIN yt_crm.t_crm_chat xiaoer ON xiaoer.type = 2 AND xiaoer.qw_user_id = total.qw_user_id and xiaoer.is_deleted = 0
INNER JOIN yt_crm.t_crm_work_phone phone ON phone.bind_qw_user_id = total.qw_user_id and phone.is_deleted = 0
...
INNER JOIN yt_crm.t_crm_chat customer ON customer.qw_external_user_id = total.qw_external_user_id and customer.is_deleted = 0
