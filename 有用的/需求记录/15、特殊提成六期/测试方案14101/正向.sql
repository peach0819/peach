...
-- 当月明细
insert overwrite table dw_salary_gmv_rule_public_d partition (dayid='$v_date',planno='14102',pltype='cur')
select
            ...
            (
                ...
                where dayid ='$v_date'
                  ...
                  and
                    ...
                    array_contains(split(shop_group,','), '13983')
                  and
...
insert overwrite table dw_salary_forward_plan_sum_d partition (dayid='$v_date',planno='14102')
select
    from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss') as update_time,--更新时间
    from_unixtime(unix_timestamp(),'yyyy-MM') as update_month,--执行月份
    --方案基础信息
    ...
    -- 累计阶梯单价返点
        ,case
             when
                         sts_target  >=  3333
                     and sts_target * 33  < 999.0                                                                         then round(sts_target * 33, 2)
             when
                         sts_target  >=  3333
                     and sts_target * 33  >= 999.0                                                         then 999.0
             when
                         sts_target  >=  2222
                     and
                         sts_target  <  3333
                     and sts_target * 22  < 999.0                                                                         then round(sts_target * 22, 2)
             when
                         sts_target  >=  2222
                     and
                         sts_target  <  3333
                     and sts_target * 22  >= 999.0                                                         then 999.0
             when
                         sts_target  >=  1111
                     and
                         sts_target  <  2222
                     and sts_target * 11  < 999.0                                                                         then round(sts_target * 11, 2)
             when
                         sts_target  >=  1111
                     and
                         sts_target  <  2222
                     and sts_target * 11  >= 999.0                                                         then 999.0
    ...
    (
        ...
        from (
                 ...
                 from (
                          ...
                          where dayid ='$v_date' and pltype='cur' and planno='14102'
                      ) t
                          left join (
