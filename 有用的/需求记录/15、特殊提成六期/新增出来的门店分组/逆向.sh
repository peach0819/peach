...
-- 历史明细
insert overwrite table dw_salary_gmv_rule_public_d partition (dayid='$v_date',planno='11435',pltype='pre')
select
   ...
   '我是测试方案1021-2' as plan_name,--方案名称
   '180778' as plan_group_id, --归属业务组id
   'DC_Test_1614205771078' as plan_group_name, --归属业务组
   business_unit,--业务域,
...
select
   ...
   refund_retreat_amount,--实货清退金额
   --发放对象--
    '【辖区】大区经理' as grant_object_type,--发放对象类型
    area_manager_id as grant_object_user_id,   --发放对象用户ID
    area_manager_name as grant_object_user_name, --发放对象用户姓名
    area_manager_dep_id as grant_object_user_dep_id,-- 发放对象部门ID
    area_manager_dep_name as grant_object_user_dep_name  --发放对象部门名称
 from
(
   select
           business_unit,--业务域,
	...
	(select *
	  ...
	  and pay_day between '20211001' and '20211031'
            ...
            array_contains(split(shop_group,','), 452367083)
                    and
...
insert overwrite table dw_salary_backward_plan_sum_mid_d partition (dayid='$v_date',planno='11435')
select
    from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss') as update_time,--更新时间
    from_unixtime(unix_timestamp(),'yyyy-MM') as update_month,--执行月份
    --方案基础信息
    ...
    --判断必须从大到小
            ...
            ,case
                    when
                        sts_target  >=  1
                                                        and sts_target * 1  < 100.0                                                                         then round(sts_target * 1, 2)
                    when
                        sts_target  >=  1
                                                        and sts_target * 1  >= 100.0                                                         then 100.0
...
from (
        select
                ...
                where dayid ='$v_date' and pltype='pre' and planno='11435'
            ) t
            left join (
...
if [ "202110" -ne "$v_cur_month" ]; then
    hive -e "
        use ytdw;
        insert overwrite table dw_salary_backward_plan_sum_d partition (dayid='$v_date',planno='11435')
        select
...
then pre_plan.commission_reward else cur_plan.commission_reward end - pre_plan.commission_reward, 2)
         ...
         (
           select  * from dw_salary_backward_plan_sum_mid_d
            where dayid ='$v_pre_month_last_day' and plan_no='11435'
            ) pre_plan

            left join
            ( select  * from dw_salary_backward_plan_sum_mid_d
              where dayid ='$v_date'  and plan_no='11435'
