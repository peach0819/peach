...
apache-spark-sql -e "
use ytdw;
CREATE TABLE if not exists ads_crm_subject_item_d (
...
subject_main as (
    SELECT subject.id,
           nvl(subject_item.item_style, 0) as item_style,
...
INSERT OVERWRITE TABLE ads_crm_subject_item_d partition (dayid='$v_date')
