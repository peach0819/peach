SELECT data.user_id as `用户id`,
	   ...
	   nvl(get_json_object(biz_value, '$.quarter_all_big_visit_cover_rate'), '')  as `当季全渠道重点门店拜访覆盖率`,
	   nvl(get_json_object(biz_value, '$.quarter_all_big_visit_cover_rate_numerator'), '')  as `当季全渠道重点门店拜访覆盖率分子`,
	   nvl(get_json_object(biz_value, '$.quarter_all_big_visit_cover_rate_denominator'), '')  as `当季全渠道重点门店拜访覆盖率分母`,
