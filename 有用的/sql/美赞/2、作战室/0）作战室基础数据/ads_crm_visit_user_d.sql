...
root_dept as (
      SELECT id,
           ...
           cnt + 1 as cnt
    FROM dept
    LATERAL VIEW posexplode(split(root_key, '_')) pos as cnt, dept_id
),
...
SELECT user_id,
       ...
       if(
           ...
           concat(prod_mdson.MapValueSorter(map_agg(if(user_id != charger, charger, NULL), cnt), 'asc'), '_', user_id),
           user_id
       ) as user_root_key,
       if(
           count(if(user_id != charger, 1, null)) > 0,
           concat(prod_mdson.MapValueSorter(map_agg(if(user_id != charger, charger_name, NULL), cnt), 'asc'), '_', user_real_name),
           user_real_name
       ) as user_name_root_key,
       prod_mdson.MapValueSorter(map_agg(if(user_id != charger, charger, NULL), cnt), 'asc') AS user_parent_root_key,
	   prod_mdson.MapValueSorter(map_agg(if(user_id != charger, charger_name, NULL), cnt), 'asc') AS user_name_parent_root_key
FROM root_user
group by user_id,
         user_real_name,
         root_key
