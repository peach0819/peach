...
stored as orc
location '/dw/ytdw/st/st_p0_subject_plan_sum_d';
...
(
  ...
  where dayid='$2' and nvl(grant_object_user_dep_id, '')<>''
) salary_sum
join(
  select no, subject_id from dwd_bounty_plan_d where dayid='$1' and nvl(subject_id,0)<>0 and status = 1
) plan
