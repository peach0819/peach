...
cur as (
    select from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss') as update_time,
           ...
           case when plan.bounty_indicator_code = 'NEW_SIGNING_ITEM_2_NEW_SING_SHOPS' then detail.sign_shop_count  --新签门店数
...
old_data as (
    SELECT planno, grant_object_user_id, sts_target, commission_reward FROM dw_salary_backward_plan_sum_mid_d WHERE dayid='20220331' AND  bounty_rule_type=2
)
