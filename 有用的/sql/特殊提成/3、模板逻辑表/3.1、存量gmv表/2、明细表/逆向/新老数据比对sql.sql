...
ord as (
    SELECT dayid,
           ...
           sum(if(business_unit not in ('卡券票','其他'), nvl(refund.refund_actual_amount,0), 0)) as refund_actual_amount,
           sum(if(business_unit not in ('卡券票','其他'), nvl(refund.refund_retreat_amount,0), 0)) as refund_retreat_amount
    FROM dw_salary_gmv_rule_public_mid_v2_d d
    LEFT JOIN refund ON d.order_id = refund.order_id
    where dayid in (replace(last_day(add_months('2022-03-31', 0)), '-', ''),
                    replace(last_day(add_months('2022-03-31', -1)), '-', ''),
                    replace(last_day(add_months('2022-03-31', -2)), '-', ''),
                    replace(last_day(add_months('2022-03-31', -3)), '-', ''),
                    replace(last_day(add_months('2022-03-31', -4)), '-', ''),
                    replace(last_day(add_months('2022-03-31', -5)), '-', ''),
                    replace(last_day(add_months('2022-03-31', -6)), '-', ''),
                    replace(last_day(add_months('2022-03-31', -7)), '-', ''),
                    replace(last_day(add_months('2022-03-31', -8)), '-', ''),
                    replace(last_day(add_months('2022-03-31', -9)), '-', ''),
                    replace(last_day(add_months('2022-03-31', -10)), '-', ''),
                    replace(last_day(add_months('2022-03-31', -11)), '-', ''))
                     AND dayid > '0'
    group by dayid,
             pay_day,
             business_unit,
...
user_admin as (
    select user_id,
           ...
           dayid
    from dwd_user_admin_d WHERE dayid > '0'
),
...
old_data as (
    SELECT * FROM dw_salary_gmv_rule_public_d WHERE dayid='20220331' AND pltype='pre'
)

SELECT t1.planno, t1.grant_object_user_id, t1.c,  t2.planno, t2.grant_object_user_id, t2.c
FROM (SELECT sum(sts_target) as c, planno,grant_object_user_id FROM new_data group by planno, grant_object_user_id) t1
         FULL JOIN (SELECT sum(sts_target) as c, planno, grant_object_user_id FROM old_data group by planno, grant_object_user_id) t2 ON t1.planno = t2.planno AND t1.grant_object_user_id = t2.grant_object_user_id
