...
insert overwrite table dw_salary_forward_plan_sum_d partition (dayid='$v_date',bounty_rule_type=2)
select
      ...
      select
         ...
         from (
               select
                     ...
                     payout_rule_type,
                     payout_config_json
                 from (select * from dw_salary_sign_item_rule_public_d where dayid ='$v_date' and pltype='cur') t1
