...
WITH shop_saler as (
    ...
    FROM (
        select *,
               row_number() over (PARTITION BY shop_id ORDER BY group_type ASC) AS rn
...
shop_subject as (
	SELECT subject_shop.shop_id,
           ...
           to_json(sort_array(collect_list(NAMED_STRUCT(
               'subject_id', subject_shop.subject_id,
               'subject_gmv', nvl(cast(subject_gmv as decimal(18,2)), 0),
               'this_month_gmv', nvl(cast(this_month_gmv as decimal(18,2)), 0),
               'last_1_month_gmv', nvl(cast(last_1_month_gmv as decimal(18,2)), 0),
               'last_2_month_gmv', nvl(cast(last_2_month_gmv as decimal(18,2)), 0),
...
SELECT shop_subject.shop_id,
       ...
       nvl(shop_saler.user_id, '') AS user_id,
       nvl(shop_saler.dept_id, 0) AS dept_id,
       nvl(shop_saler.parent_dept_id, 0) AS parent_dept_id,
       nvl(shop_saler.parent_2_dept_id, 0) AS parent_2_dept_id,
       nvl(subject_ids, ''),
       nvl(subject_count, 0),
       nvl(subject_data, ''),
...
left join shop on shop.shop_id=shop_subject.shop_id
LEFT JOIN shop_visit ON shop_subject.shop_id = shop_visit.shop_id
LEFT JOIN shop_pay ON shop_subject.shop_id = shop_pay.shop_id