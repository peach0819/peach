use ytdw;
CREATE TABLE if not exists st_p0_subject_saler_vs_h(
...
with shop_saler as (
    ...
    where dayid='$v_date'
    AND get_json_object(user_id, '$.vs') != ''
)

insert overwrite table st_p0_subject_saler_vs_h partition (dayid='$v_date')
...
(
  ...
  on subject.id=subject_shop.subject_id
