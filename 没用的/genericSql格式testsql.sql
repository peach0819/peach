...
CREATE TABLE if not exists st_p0_subject_rule_detail_d (
    ...
    stored as orc
    location '/dw/ytdw/st/st_p0_subject_rule_detail_d';


insert overwrite table ytdw.st_p0_subject_rule_detail_d partition (dayid='$v_date')
select
...
from
    (
        select
            ...
            sts_target_name,
            sts_target
        from dw_salary_gmv_rule_public_d where dayid='$v_date'  and nvl(grant_object_user_id, '')<>''
                                             ...
                                             (
                                                 ...
                                                 from
                                                     (
                                                         ...
                                                         where dayid='$v_date' and nvl(grant_object_user_id, '')<>''
                                                         --where dayid='20200430' and nvl(grant_object_user_id, '')<>''
                                                     )t
                                             )b
                                     )b1
                             )t1
        ...
        join(
        select no, subject_id from dwd_bounty_plan_d where dayid='$v_date' and nvl(subject_id,0)<>0 and status = 1
    ) plan
            on plan.no=plan_data.planno
        left join(
        select subject_id,shop_id,is_object_shop from dwd_p0_subject_shop_d where dayid='$v_date'
    ) subject_shop
