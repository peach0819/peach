...
public class LC654_maximum_binary_tree {
    ...
    private TreeNode handle(int beginIndex, int endIndex) {
        if (beginIndex > endIndex) {
            return null;
        }
        int maxVal = nums[beginIndex];
        ...
        node.val = maxVal;
        node.left = handle(beginIndex, maxIndex - 1);
        node.right = handle(maxIndex + 1, endIndex);
        return node;
    }
}
