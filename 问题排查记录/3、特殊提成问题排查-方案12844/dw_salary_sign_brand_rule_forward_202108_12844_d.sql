...
insert overwrite table dw_salary_sign_brand_rule_public_d partition (dayid='$v_date',planno='12844',pltype='cur')
select
   from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss') as update_time,--更新时间
   from_unixtime(unix_timestamp(),'yyyy-MM') as update_month,--执行月份
   '当月方案' as plan_type ,--明细类型
   --方案基础信息,
   '202108' as plan_month,--方案月份
   '2021-08-01~2021-08-31' as plan_pay_time,--方案时间
   '8月商品组P0-奶粉-小诸葛-安徽省区-儿童粉-toBD' as plan_name,--方案名称
...
insert overwrite table dw_salary_forward_plan_sum_d partition (dayid='$v_date',planno='12844')
select
    from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss') as update_time,--更新时间
    from_unixtime(unix_timestamp(),'yyyy-MM') as update_month,--执行月份
    --方案基础信息
        ...
        --提成奖品类型
            ...
            '金额' as commission_reward_type
                ...
                ,case
                    when
                        sts_target  >=  5
                                                    then round(sts_target * 100, 2)
                            when
                        sts_target  >=  1
                 and                         sts_target  <  5
                                                    then round(sts_target * 70, 2)
...
from (
    ...
    count(distinct case  when  is_leave='否' and is_succ_sign='是' then concat('_',shop_id,brand_id)  else null end ) as sts_target--统计指标数值
        ...
        (
            ...
            where dayid ='$v_date' and pltype='cur' and planno='12844'
        ) t
        left join
