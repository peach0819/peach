...
if [ "202106" -ne "$v_cur_month" ]; then
    hive -e "
        use ytdw;
        insert overwrite table dw_salary_backward_plan_sum_d partition (dayid='$v_date',planno='12381')
        select
...
then pre_plan.commission_reward else cur_plan.commission_reward end - pre_plan.commission_reward, 2)
         ...
         (
           select  * from dw_salary_backward_plan_sum_mid_d
            where dayid ='$v_pre_month_last_day' and plan_no='12381'
            ) pre_plan

            left join
            ( select  * from dw_salary_backward_plan_sum_mid_d
              where dayid ='$v_date'  and plan_no='12381'
