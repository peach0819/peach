...
order_base as (
    ...
    from order_info
    left join afs_refund on order_info.order_id = afs_refund.order_id
),
...
order_base_info as (
    ...
    and sp_id is null  --排除服务商数据
    and store_type not in (9,10)  --排除员工店
...
pro_shop_stats as (
    ...
    from order_base
    where sp_id is null
    and nvl(store_type,100) not in (3,9,10,11)  --排除员工店、伙伴店11
    and item_style_freezed=1
...
--省份达标线
pro_mark_line as (
    select shop_pro_id,new_sign_shop_mark_line
    from dwd_salary_pro_mark_line_d
    where dayid='$v_date'
),
--门店本月净支付金额最大的一天，以及相关信息
pro_shop_pure_pay_amount_info as (
    select shop_id,
           service_user_id,
           service_user_name,
    ...
    from pro_shop_stats
    left join pro_mark_line on pro_mark_line.shop_pro_id=pro_shop_stats.shop_pro_id
    where pure_pay_amount>=pro_mark_line.new_sign_shop_mark_line
),
--门店当月的最大支付信息
cur_month_shop_top_pay_amount_info as (
    select
        shop_id,
        service_user_id,
        service_user_name,
        ...
        refund_actual_amount,
        pure_pay_amount
    from pro_shop_pure_pay_amount_info
    where rn=1 and substr(pro_shop_pure_pay_amount_info.pay_day,1,6)='$v_cur_month'
),
--首次下b类时间
min_pay_b_day as (
    select shop_id, min(substr(pay_time,1,8)) min_pay_day
    from order_base
    where sp_id is null
    and nvl(store_type,100) not in (3,9,10,11) --排除员工店、伙伴店11
    and item_style_freezed=1
...
shop_new_sign as (
    select
        shop_base.shop_id,
        cur_month_shop_top_pay_amount_info.service_user_id,
        cur_month_shop_top_pay_amount_info.service_user_name,
        cur_month_shop_top_pay_amount_info.pay_day,
        cur_month_shop_top_pay_amount_info.pure_pay_amount,
        shop_base.approve_status_name
    from cur_month_shop_top_pay_amount_info
    inner join shop_base on cur_month_shop_top_pay_amount_info.shop_id=shop_base.shop_id
    left join min_pay_b_day on min_pay_b_day.shop_id=shop_base.shop_id
    where substr(min_pay_b_day.min_pay_day,1,6)='$v_cur_month'
),
...
--本月线下退款合计信息 trade_no -> 合计
cur_month_offline_refund as (
    select trade_no,sum(refund_actual_amount) refund_actual_amount
    from dw_offline_refund_d
    where dayid='$v_date'
    group by trade_no
),
--上月线下退款合计信息 trade_no -> 合计
last_month_offline_refund as (
    select trade_no,sum(refund_actual_amount) refund_actual_amount
    from dw_offline_refund_d
...
trade_order_offline_refund as (
    select order_shop.order_id,
           ...
           last_month_offline_refund.refund_actual_amount*(order_shop.item_actual_amount/trade_shop_total.item_actual_amount) last_month_refund_actual_amount
    from order_shop
    left join cur_month_offline_refund on cur_month_offline_refund.trade_no=trade_shop.trade_no
    left join last_month_offline_refund on last_month_offline_refun.trade_no=trade_shop.trade_no
),
--门店交易信息 (订单-线下退款订单)
shop_trade as (
    select shop_id,
           sum(case when substr(pay_time,1,6)='$v_pre_month' then total_pay_amount else 0 end)-sum(pre_mnt_refund_actual_amount)-nvl(sum(nvl(trade_order_offline_refund.last_month_refund_actual_amount,0)),0) as last_month_big_b_pure_gmv,
           sum(case when substr(pay_time,1,6)='$v_cur_month' then total_pay_amount else 0 end)-sum(cur_mnt_refund_actual_amount)-nvl(sum(nvl(trade_order_offline_refund.cur_month_refund_actual_amount,0)),0) as cur_month_big_b_pure_gmv
    from shop_trade_order
    left join trade_order_offline_refund on trade_order_offline_refund.order_id=shop_trade_order.order_id
    group by shop_id
),
--上月省份达标线
last_month_pro_mark_line as (
    select shop_pro_id,reach_shop_mark_line
    from dwd_salary_pro_mark_line_d
    where dayid='$v_pre_month_last_day'
),
--本月省份达标线
cur_month_pro_mark_line as (
    select shop_pro_id,reach_shop_mark_line
    from dwd_salary_pro_mark_line_d
    where dayid='$v_date'
),
...
shop_bd_stats as (
    select shop_id,
           ...
           sum(case when substr(pay_time,1,6)='$v_pre_month' then total_pay_amount else 0 end)-nvl(sum(pre_mnt_refund_actual_amount),0) as b_pure_gmv_04
    from order_base
    where store_type not in (9,10)
    and item_style=1
    group by shop_id, bd_user_id
),
--单个销售业绩最好的门店top10
shop_top_10 as (
    select shop_id
...
shop_order as (
    select
        shop_base.shop_id,
        shop_trade.last_month_big_b_pure_gmv,
        case when shop_trade.last_month_big_b_pure_gmv>=cur_month_pro_mark_line.reach_shop_mark_line then '达标' else '否' end as last_month_is_reach,
        last_month_pro_mark_line.reach_shop_mark_line as last_month_reach_goal,
        shop_trade.cur_month_big_b_pure_gmv,
        case when shop_trade.cur_month_big_b_pure_gmv>=cur_month_pro_mark_line.reach_shop_mark_line then '达标' else '否' end as cur_month_is_reach,
        cur_month_pro_mark_line.reach_shop_mark_line as cur_month_reach_goal,
        nvl(shop_trade.cur_month_big_b_pure_gmv,0)-cur_month_pro_mark_line.reach_shop_mark_line as cur_month_balance,
        shop_stats.max_big_b_date,
        case
          when shop_stats.a_or_small_b_max_date>='$v_60_days_ago' and shop_stats.a_or_small_b_max_date<='$v_date' and shop_stats.max_big_b_date is null then '未下过B近60天下过A'
          when shop_stats.a_or_small_b_max_date is not null and shop_stats.max_big_b_date is null then '未下过B下过A'
          when shop_stats.max_big_b_date is not null then '下过B门店'
          else '未下单门店'
        end as shop_pay_status,
        shop_max_order.last_month_max_big_g_pure_amount,
        shop_max_order.cur_month_max_big_g_pure_amount,
        case when shop_top_10.shop_id is not null then 'BDtop10门店' else '否' end as is_bd_top10,
        from_unixtime(unix_timestamp()) as update_time,
        shop_stats.today_b_gmv,
        today_exclude_sp_b_gmv,
        pre_month_exclude_sp_b_gmv,
        cur_month_exclude_sp_b_gmv
    from shop_base
    left join shop_trade on shop_trade.shop_id=shop_base.shop_id
    left join last_month_pro_mark_line on last_month_pro_mark_line.shop_pro_id=shop_base.shop_pro_id
    left join cur_month_pro_mark_line on cur_month_pro_mark_line.shop_pro_id=shop_base.shop_pro_id
    left join shop_stats on shop_stats.shop_id=shop_base.shop_id
    left join shop_max_order on shop_max_order.shop_id=shop_base.shop_id
    left join shop_top_10 on shop_top_10.shop_id=shop_base.shop_id
),
...
--门店首次下大B订单的时间
shop_big_b_min_day as (
    select shop_id, min(substr(pay_time,1,8)) min_pay_day
    from order_base
    where sp_id is null
    and nvl(store_type,100) not in (3,9,10,11) --排除员工店、伙伴店11 20200505
    and item_style_freezed=1
    group by shop_id
),
...
brand_shop as (
    ...
    from brand_shop_orders
    left join salary_user on salary_user.user_name=brand_shop_orders.service_user_name
    where substr(shop_brand_sign_day,1,6)='$v_cur_month'
    and pure_pay_amount>=1000
    and salary_user.user_name is not null
),

--有效新开门店  过滤有效新开（沉淀一个月数据后可做替换，也可直接直接执行上个月任务）
new_sign_shop as (
    select shop_id
...
shop_all_order as (
    ...
    --20200728 退款改造
    from order_base_info
    left join offline_refund on offline_refund.order_id=order_base_info.order_id
    group by order_base_info.shop_id
),
--门店B类订单净支付统计
shop_pure_pay_amount_info as (
    select shop_id,
           pay_day,
           coalesce(ytdw.get_service_info('service_feature_id:5',service_info_freezed,'service_user_id'),ytdw.get_service_info('service_feature_id:1',service_info_freezed,'service_user_id'),ytdw.get_service_info('service_feature_id:4',service_info_freezed,'service_user_id')) as service_user_id,
           coalesce(ytdw.get_service_info('service_feature_id:5',service_info_freezed,'service_user_name'),ytdw.get_service_info('service_feature_id:1',service_info_freezed,'service_user_name'),ytdw.get_service_info('service_feature_id:4',service_info_freezed,'service_user_name')) as service_user_name,
...
--门店服务人员销售数据统计, 用于门店激活信息判断
shop_service_user as (
    select shop_id,
           pay_day as cur_month_min_b_pay_day,
           service_user_id,
           service_user_name,
           service_department_name,
           b_pure_pay_amount,
           row_number()over(partition by shop_id order by pay_day asc,b_pure_pay_amount desc) as rn
    from shop_pure_pay_amount_info
    where b_pure_pay_amount>=1000
),
--门店的所有统计信息
shop_info as (
    select shop_all_order.shop_id,
    ...
    from shop_all_order
    left join shop_service_user on shop_service_user.shop_id=shop_all_order.shop_id  and rn=1
...
shop_silent_info as (
    ...
    from shop_silent
    left join brand_shop on shop_silent.shop_id=brand_shop.shop_id and rn=1
    left join new_sign_shop on new_sign_shop.shop_id=shop_silent.shop_id
)
...
select shop_base.shop_id,
       ...
       from_unixtime(unix_timestamp()) as update_time,
       shop_silent_info.max_last_b_date,
       shop_silent_info.all_before_last_month_b_gmv,
       shop_silent_info.is_silent,
       shop_silent_info.is_silent_type,
       shop_silent_info.cur_month_min_b_pay_day,
       shop_silent_info.service_user_id as is_silent_service_user_id,
       shop_silent_info.service_user_name as is_silent_service_user_name,
       shop_silent_info.is_brand_shop,
       shop_silent_info.is_brand_shop_type,
       shop_silent_info.brand_name,
       shop_silent_info.is_brand_pay_day,
       shop_silent_info.is_brand_service_user_id,
       shop_silent_info.is_brand_service_user_name,
...
from shop_base
left join shop_new_sign on shop_new_sign.shop_id=shop_base.shop_id
left join shop_order on shop_order.shop_id=shop_base.shop_id
left join shop_level on shop_level.shop_id=shop_base.shop_id
left join shop_visit on shop_visit.shop_id=shop_base.shop_id
left join shop_visit_history on shop_visit_history.shop_id=shop_base.shop_id
left join shop_big_b_min_day on shop_big_b_min_day.shop_id=shop_base.shop_id
left join shop_silent_info on shop_silent_info.shop_id=shop_base.shop_id