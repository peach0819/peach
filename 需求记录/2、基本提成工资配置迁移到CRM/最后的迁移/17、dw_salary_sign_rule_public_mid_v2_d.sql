...
--默认指标--
--默认指标--
order1.gmv - nvl(refund.refund_actual_amount,0) as gmv_less_refund,  --实货gmv-退款,
order1.pay_amount,--实货支付金额
order1.pay_amount - nvl(refund.refund_actual_amount,0)  as pay_amount_less_refund,--实货支付金额-退款
gmv,--实货gmv,
...
--特殊订单表
left join spec_order on order1.trade_no= spec_order.trade_no
--门店表
left join shop on order1.shop_id=shop.shop_id
where
